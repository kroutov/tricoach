...
- **Garmin nécessite désormais une double authentification (MFA)**, non gérée par la librairie `garmin-connect` utilisée (TODO jamais implémenté dans son code source) — laissé de côté à la demande de l'utilisateur plutôt que de s'engager dans une rétro-ingénierie non testable sans compte réel.
- **Page Activités ajoutée** (`GET /me/activities`) : historique de toutes les activités synchronisées, avec dédoublonnage inter-sources à l'ingestion (même sport à ±5 min entre deux fournisseurs, heuristique portée depuis `HealthDataAggregator.swift` côté iOS) — prépare le terrain pour Garmin+Strava sans doublons une fois le MFA débloqué. Détail dans "Post-lancement" de [`tricoach-web/README.md`](tricoach-web/README.md#statut).
