...
- Backend : `cd tricoach-backend && npm test` (110 tests unitaires + intégration contre une vraie base Postgres de test, y compris Strava/Garmin/HealthKit/webhooks avec API externes simulées, flux ICS calendrier, sécurité — rate limiting/CORS/headers/JWT — auth email/mot de passe/Google, et config fail-fast).
